...
async def handle_message(message: types.Message):
    ...
    if text == "📋 Возражения":
        ...
        await message.answer(
            ...
            "Опишите ситуацию клиента — я дам подробный разбор, скажу можно ли списать долги и что делать дальше.",
    ...
    # ==================== ПОЛНЫЙ СКРИПТ ПЕРВОГО РАЗГОВОРА ====================
        ...
        # Вопрос к ИИ с подгрузкой rules.txt
...
Вот актуальные правила и данные от владельца компании, на которые ты всегда должен ориентироваться:

{rules_content}

Ты отвечаешь ИСКЛЮЧИТЕЛЬНО на вопросы, связанные с банкротством физических лиц.
Если вопрос не по теме — отвечай строго одной фразой: «Я специализируюсь исключительно на банкротстве физических лиц. По данному вопросу ответить не могу.»

Если вопрос касается банкротства — отвечай по следующим правилам:
1. Начинай сразу с **Прямого ответа** (можно списать / нельзя списать + суть).
2. Давай максимально подробный и развёрнутый ответ.
3. Используй структуру:
   - Прямой ответ
   - Подробное объяснение механизма
   - Важные нюансы и риски
   - Варианты действий
   - Защита имущества (если упоминается)
4. Если информации недостаточно — в конце задавай конкретные уточняющие вопросы нумерованным списком.
...
if __name__ == "__main__":
    ...
    print("✅ Бот запущен: все возражения + rules.txt подключён к GigaChat")
