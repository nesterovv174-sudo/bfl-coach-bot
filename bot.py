...
async def handle_message(message: types.Message):
    ...
    if text == "📋 Возражения":
        ...
        await message.answer(
            ...
            "Опишите ситуацию клиента — я скажу, можно ли списать долги, какие риски и что делать дальше.",
...
async def handle_template_objection(message: types.Message, objection: str):
    templates = {
        ...
        "🏠 Заберут квартиру": (
            "🏠 **Заберут квартиру**\n\n"
            "**Вариант 1:**\n«Это один из самых частых страхов. Единственное жильё, в котором вы проживаете, по закону полностью защищено и не может быть реализовано.»\n\n"
            "**Вариант 2:**\n«Я вас понимаю, эта мысль пугает. На практике в большинстве случаев единственное жильё остаётся у клиента.»"
        ),
        "🤔 Нужно подумать": (
            "🤔 **Нужно подумать**\n\n"
...
Если вопрос не касается банкротства физических лиц, ты отвечаешь строго одной фразой:
...
1. Начинай ответ сразу с **Прямого ответа** — можно ли списать долги в данной ситуации или нельзя.
2. Всегда давай максимально развёрнутый и подробный ответ (минимум 450–700 слов).
3. Обязательная структура ответа:
   - **Прямой ответ** — можно списать / нельзя списать + краткая суть
   - **Подробное объяснение механизма** — как это работает на практике в 2026 году
   - **Важные нюансы и риски** — все подводные камни и последствия
   - **Варианты действий** — пошаговый план, какие документы нужны, куда обращаться
   - **Защита имущества** — подробно разбери все законные способы сохранить имущество
4. Если для точного ответа не хватает информации (сумма долгов, состав кредиторов, наличие имущества, доход, регион, просрочка и т.д.) — в конце ответа обязательно задай конкретные уточняющие вопросы нумерованным списком.
Ты отлично знаешь актуальное законодательство 2026 года и всегда говоришь только о легальных способах.
"""
...
if __name__ == "__main__":
    ...
    print("✅ Бот запущен: все возражения + обновлённый промпт для GigaChat")
