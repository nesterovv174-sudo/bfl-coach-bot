...
# ==================== GIGACHAT С ЖЁСТКИМ ПРИНУЖДЕНИЕМ ИСПОЛЬЗОВАТЬ rules.txt ====================
...
Ты должен использовать **только** данные из следующего блока правил. Никогда не используй другие цифры прожиточного минимума или другие значения.

=== НАЧАЛО ПРАВИЛ ИЗ ФАЙЛА ===
{rules_content}
...
2. Обязательно рассчитывай конкурсную массу **только** на основе данных из блока выше. Все расчёты делай **только в рублях**.
...
Отвечай профессионально, чётко и подробно. Используй нумерацию и списки.
"""
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                json={
                    ...
                    "temperature": 0.4,   # сделали ниже, чтобы был более точным и следовал правилам
                    "max_tokens": 2200
                },
                ssl=False
            ) as resp:
...
if __name__ == "__main__":
    ...
    print("✅ Бот запущен: rules.txt подключён жёстко + акцент на расчёт и имущество")
