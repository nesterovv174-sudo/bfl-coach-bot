...
async def handle_message(message: types.Message):
    ...
    if text == "📋 Возражения":
        ...
        await message.answer(
            ...
            "Опишите ситуацию клиента (доход, долги, имущество и т.д.) — я рассчитаю конкурсную массу, покажу примеры и скажу, можно ли списать долги.",
...
async def handle_template_objection(message: types.Message, objection: str):
    # Здесь можно оставить твои текущие тексты (по 2 варианта)
    templates = {
        "💰 Дорого / Цена": "💰 **Дорого / Цена**\n\n**Вариант 1:**\n«Понимаю вас очень хорошо... (твой текст)»\n\n**Вариант 2:**\n«...»",
        # Добавь остальные возражения по необходимости
    }
...
# ==================== GIGACHAT С РАСЧЁТОМ КОНКУРСНОЙ МАССЫ ====================
async def handle_gigachat(message: types.Message):
    ...
    try:
        ...
        rules_content = "Правила и актуальные данные не найдены (файл rules.txt отсутствует)."
...
Вот актуальные правила и цифры от владельца компании, на которые ты **обязательно** должен ориентироваться при каждом ответе:
...
2. Обязательно рассчитывай конкурсную массу на основе дохода и прожиточного минимума из rules.txt.
3. Показывай **пример расчёта** конкурсной массы (цифры и пояснения).
4. Чётко говори, подходит ли человек под списание при его доходе.
5. Давай максимально подробный и развёрнутый ответ.
6. Если информации недостаточно — в конце задавай конкретные уточняющие вопросы нумерованным списком.

Отвечай профессионально, чётко и очень подробно. Используй нумерацию и списки.
"""
...
if __name__ == "__main__":
    ...
    print("✅ Бот запущен: rules.txt подключён + расчёт конкурсной массы")
