...
async def handle_message(message: types.Message):
    ...
    if text == "📋 Возражения":
        ...
        await message.answer(
            ...
            "Опишите ситуацию клиента (доход, долги, имущество и т.д.) — я рассчитаю конкурсную массу по твоим правилам и скажу можно ли списать долги.",
...
# ==================== GIGACHAT — ЖЁСТКИЙ ПРОМПТ С ПРИНУЖДЕНИЕМ ====================
...
**САМОЕ ВАЖНОЕ ПРАВИЛО (обязательно соблюдай всегда):**
Ты **должен** использовать **только** данные из следующего блока. Никогда не используй другие цифры прожиточного минимума или другие значения.

=== НАЧАЛО ПРАВИЛ ИЗ ФАЙЛА rules.txt ===
{rules_content}
=== КОНЕЦ ПРАВИЛ ИЗ ФАЙЛА ===

Все расчёты делай **только в рублях**. Никогда не используй доллары или другие валюты.

Правила ответа:
1. Всегда начинай с **Прямого ответа**: можно ли списать долги в данной ситуации или нельзя.
2. Обязательно рассчитывай конкурсную массу на основе данных из rules.txt и показывай пример расчёта в рублях.
3. Подробно разбирай **защиту имущества** и все возможные законные варианты его спасения.
4. Давай очень подробный ответ.
5. Если информации недостаточно — в конце задавай конкретные уточняющие вопросы нумерованным списком.

Отвечай профессионально, чётко и подробно.
"""
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                json={
                    ...
                    "temperature": 0.3,   # сильно снизили, чтобы меньше фантазировал
                    "max_tokens": 2200
                },
                ssl=False
            ) as resp:
...
if __name__ == "__main__":
    ...
    print("✅ Бот запущен: максимально жёсткий промпт + принуждение использовать rules.txt")
